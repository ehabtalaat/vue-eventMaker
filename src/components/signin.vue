<template>
    ...
    <img src="1.png" class="avatar">
        ...
        <form>
            ...
            <input type="submit" name="" value="Login" @click.prevent="signin">
            <div class="alert alert-danger" v-if="error.message"> {{error.message}}</div>
            

            <span>Don't have an account? <router-link to="/register" class="signup">sign up</router-link></span>
        </form>
       </div> 
	</div>
</template>
...
import {fireApp} from "../firebaseApp";
	...
	methods:{
 	signin(){
 		fireApp.auth().signInWithEmailAndPassword(this.email,this.password)
...
{
    cursor: pointer;
      background-image: linear-gradient(to right, red , rgba(255,0,0,.8));
    color: #000;
